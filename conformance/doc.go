...
// schema_test.go, instance_test.go) keep their plain t.Skipf on a missing suite,
// unconditionally. They are deliberately treated differently from
// TestConformance: each drives a named cohort of suite documents to prove one
// executor decides them for the right reason, so skipping them loses unit
