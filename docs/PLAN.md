...
#182 and #183 draining — is met; all four are closed. Follow the pattern
that worked for M4 (#79 → #167–#183): model and infoset shape slices
first, then a lane bring-up slice that produces a real `instance` number
early (the #175 analogue), then the validator fan-out.

**Working band** (dependency-ordered top of the `ready` queue, so a
session need not scan 133 issues): *not published in this pass.* This
Status section was written by a process refactor, not by a `/backlog`
run; the next `/backlog` publishes the band with the rest of the
section.
...
values. **`instance` lane.** Filed as an epic and deliberately uncarved
until now; see the Status section for the carve.
