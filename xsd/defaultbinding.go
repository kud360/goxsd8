...
//     declaration·, so it wins over any wildcard.
//   - cases 4/5/6: otherwise, if c's {attribute wildcard} admits n, the wildcard's
//     {process contents} keyword.
//
// ok is false when c admits no attribute of that name at all — no matching use
// and no admitting wildcard — in which case there is no binding to compare and
// the CALLER charges the failure. Case 1 (·governing element declaration·) is
// the element half and is not reachable here.
//
