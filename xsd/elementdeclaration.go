...
//
// Owner is the token of the OWNERSHIP EDGE that reaches this container, one mint
// per edge: it identifies the container to its own nested local declarations and
// to nothing else. Two structurally distinct containers therefore never share it,
// which is the invariant to rely on.
//
// It COINCIDES with the container's {context} identity (§3.4.1 ctd-context) only
// where the owner reaches at most one anonymous type through that {context}, and
// two of the three edges are of that kind:
//
//   - an element declaration's OWN inline <complexType> child (§3.4.2.1
//     dcl.ctd.common); Owner is the owning ELEMENT DECLARATION's minted identity,
//     which is also the type's {context};
//   - the {name}-·absent· original of a redefinition (§4.2.4 src-expredef clause
//     1.1); Owner is the REDEFINING COMPLEX TYPE's minted identity, which is also
//     the original's {context}, and no element declaration is involved at all.
//
// The third does NOT coincide, and reading the coincidence as the invariant is
// what would collapse it: the anonymous <complexType> of an <xs:alternative>
// (§3.12.2 declare-ta) has the enclosing element declaration as its {context}
// too, exactly as that element's own inline type does, because §3.4.2.1 climbs to
// the nearest ancestor <element> unconditionally on child position. One element
// may own several such types, so the {context} is shared and cannot identify any
// one of them; the edge carries its own mint instead (parser/produce_complex.go's
// typeAlternativeOwnedComplexType).
//
// Comparing a {context} with the owner's identity is how
// NewElementDeclarationOwningTypes, NewComplexTypeOwningBase and
// NewAnonymousComplexTypeOwningBase each check that the anonymous type points
// back at its own owner — a check on the {context}, not on this field. A chained
// <redefine> stacks two ownership edges — a redefining type owns an original
// which owns another — and mints a token for each, so the two containers stay
// distinguishable (#585).
//
...
// complex type of an inline <complexType> child or of an <xs:alternative>; both
// reject the states e-props-correct (§3.3.6.1) clauses 1 and 3 forbid so they
// are unrepresentable (STYLE T1). ElementDeclaration is immutable after
// construction.
...
// same footing, in EVERY slot a declaration can own one through — the
// {type definition} itself, an {alternatives} member of typeTable, and its
// {default type definition} (ownedTypeSlots). Every one of those anonymous types
// carries a {context} naming THIS declaration (§3.4.2.1 dcl.ctd.common), and this
// entry point takes no identity to check that back-pointer against. Build it
// through NewElementDeclarationOwningTypes, which does.
//
...
func NewElementDeclaration(loc xsderr.Loc, name QName, typeDefinition TypeDefinitionOrRef, typeTable *TypeTable, scope Scope, valueConstraint *ValueConstraint, nillable bool, identityConstraints []IdentityConstraint, substitutionGroupAffiliations []QName, substitutionGroupExclusions []DerivationMethod, abstract bool, disallowedSubstitutions []DerivationMethod, annotations []Annotation) (ElementDeclaration, error) {
	for _, s := range ownedTypeSlots(typeDefinition, typeTable) {
		if _, owns := ownedComplexType(s.ref); !owns {
			continue
		}
		return ElementDeclaration{}, xsderr.New(xsderr.RuleComponentInvariant, loc,
			"element declaration %s %s is an InlineTypeDefinition wrapping a ComplexType, but a declaration that OWNS an anonymous complex type must be built through NewElementDeclarationOwningTypes, the only entry point that can check the type's {context} back-pointer (§3.4.2.1 dcl.ctd.common) against the declaration's own identity", name, s.property)
	}
...
// (§3.3.2.1 dcl.elt.common clause 1) and the inline <complexType> of any
// <xs:alternative> among its {type table} (§3.12.2 declare-ta). It is
// NewElementDeclaration's parameter list preceded by the declaration's own minted
// identity; every other parameter, check, copy, and rejection is identical,
// because both entry points run one shared core.
//
// It is the ONLY construction path for a declaration owning an anonymous complex
// type — NewElementDeclaration rejects that shape outright, in every slot — and
// it exists to make ONE state unrepresentable that no shape check could reach: an
...
// items", unconditionally on child position, so ONE identity — the one the caller
// minted for this declaration — is what every owned type's {context} must carry
// (compared with ==; see ComponentID for why reflect.DeepEqual cannot see it),
// however many of them there are. It adds three rejections, all charged to
...
//     one case and it yields an Element Declaration, so a <complexType> under an
//     <element> is never contexted in a complex type.
//
...
// together with its reader (see ComponentID). It is NOT the token the anonymous
// types' own nested local declarations report as {scope}.{parent} either, except
// for the single type a declaration reaches through its own {type definition}
// slot — see AnonymousComplexTypeScopeParent for the per-edge mint.
func NewElementDeclarationOwningTypes(loc xsderr.Loc, id ComponentID, name QName, typeDefinition TypeDefinitionOrRef, typeTable *TypeTable, scope Scope, valueConstraint *ValueConstraint, nillable bool, identityConstraints []IdentityConstraint, substitutionGroupAffiliations []QName, substitutionGroupExclusions []DerivationMethod, abstract bool, disallowedSubstitutions []DerivationMethod, annotations []Annotation) (ElementDeclaration, error) {
	if id == (ComponentID{}) {
		return ElementDeclaration{}, xsderr.New(xsderr.RuleComponentInvariant, loc,
			"element declaration %s may own anonymous complex types but carries an unminted identity, which their {context} back-pointers could not name; mint one with NewComponentID", name)
	}
...
// ownedTypeSlot is one {type definition} slot of an element declaration together
// with the §3 property path naming it, as a rejection message spells it.
type ownedTypeSlot struct {
	property string
	ref      TypeDefinitionOrRef
}

// ownedTypeSlots enumerates, in property order, every slot of an element
// declaration that can hold an anonymous complex type the declaration OWNS: the
// {type definition} itself (§3.3.2.1 dcl.elt.common clause 1), each
// {alternatives} member of the {type table} and its {default type definition}
// (§3.12.2 declare-ta). It is the ONE enumeration of that set (STYLE T4) —
// checkOwnedTypeContexts and NewElementDeclaration's own rejection both walk it,
// so a slot added later is covered by both or by neither.
func ownedTypeSlots(typeDefinition TypeDefinitionOrRef, typeTable *TypeTable) []ownedTypeSlot {
	...
	for i, alt := range typeTable.Alternatives() {
		slots = append(slots, ownedTypeSlot{
			property: fmt.Sprintf("{type table}.{alternatives}[%d].{type definition}", i),
...
// ownedComplexType unwraps the anonymous Complex Type Definition a slot owns
// outright — the one arm × kind pair that carries a §3.4.1 {context} back-pointer
// to check. ok is false for an inline SIMPLE type, whose §3.16.1 std-context this
// package does not model (typedefinition.go), and for every other arm, which owns
// nothing.
...
// checkOwnedTypeContexts rejects an owned anonymous complex type whose {context}
// does not name the owning declaration's identity id, TOTALLY over the slots
// ownedTypeSlots enumerates.
func checkOwnedTypeContexts(loc xsderr.Loc, id ComponentID, name QName, typeDefinition TypeDefinitionOrRef, typeTable *TypeTable) error {
	for _, s := range ownedTypeSlots(typeDefinition, typeTable) {
		if err := checkOwnedTypeContext(loc, id, name, s.property, s.ref); err != nil {
			return err
		}
	}
	return nil
...
// checkOwnedTypeContext rejects one slot's owned anonymous complex type when its
// {context} (§3.4.1 ctd-context) does not name the owning declaration's identity
// id; property names the slot in the rejection. A slot that owns no complex type
// passes, and so does an ABSENT {context} — that means the definition is NAMED,
// which the shared core's checkTypeDefinitionOrRef rejects with the message that
// fits it, so this checker passes it through rather than duplicating that verdict
// (STYLE T4).
//
	...
	case ElementDeclarationContext:
		if c.ID() != id {
			return xsderr.New(xsderr.RuleComponentInvariant, loc,
				"element declaration %s owns through its %s an anonymous complex type whose {context} names a DIFFERENT element declaration, but §3.4.2.1 dcl.ctd.common makes the {context} of a <complexType> under an <element> that element's own declaration; mint one identity per declaration and pass it to every type it owns", name, property)
		}
		return nil
	case ComplexTypeDefinitionContext:
		return xsderr.New(xsderr.RuleComponentInvariant, loc,
			"element declaration %s owns through its %s an anonymous complex type whose {context} is a ComplexTypeDefinitionContext, but §3.4.2.1 dcl.ctd.common has exactly one case and it yields an Element Declaration, so a <complexType> under an <element> is never contexted in a complex type definition", name, property)
	default:
...
//
// PRECONDITION, enforced by its TWO CALLERS and not by itself: every slot
// ownedTypeSlots enumerates — the typeDefinition and the typeTable's own two —
// that holds an InlineTypeDefinition wrapping a ComplexType arrived through
