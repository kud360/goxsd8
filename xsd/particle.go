...
// absent (zero) QName, charged to xsderr.RuleComponentInvariant rather than to a
// numbered p-props-correct clause: clause 1 is already satisfied by the present
// {term}, and no spec clause — p-props-correct, ref.elt.global (§3.3.2.4),
// src-resolve (§3.17.6.2) — textually forbids an empty ref QName, because that
...
// unflagged through both construction and finalize. NewAttributeUse rejects the
// analogous AttributeDeclarationRef state on the same footing.
//
